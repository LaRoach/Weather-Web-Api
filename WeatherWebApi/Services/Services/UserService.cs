...
namespace WeatherWebApi.Services.Services
{
    public class UserService : IUserService
    {
        ...
        private async Task<string> UploadImage(UserRegisterRequest user)
        {
            ...
            using (var client = new AmazonS3Client(Environment.GetEnvironmentVariable("AWSKeyId") ?? _config.GetSection("AWSKeyId").Value, Environment.GetEnvironmentVariable("AWSSecretAccessKey") ?? _config.GetSection("AWSSecretAccessKey").Value, RegionEndpoint.APSouth1))
            {
                using (var newMemoryStream = new MemoryStream())
                {
                    ...
                    uniqueFileName = $"https://{Environment.GetEnvironmentVariable("AWSBucketName") ?? _config.GetSection("AWSBucketName").Value}.s3.ap-south-1.amazonaws.com/" + uploadRequest.Key;
                }
            }
